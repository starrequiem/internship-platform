...
## 2026-09-05 管理后台初始化与修复记录

### 已完成

- 在 Railway 应用服务 Console 执行 `npm run db:init`，确认管理员 `admin` 已创建。
- 后台统计已验证可读取生产库，当前用户数为 2（管理员及 QA 测试用户）。
- 修复后台岗位管理列表对历史数据使用内连接的问题；历史导入岗位现在可显示，缺失发布者显示“历史发布者”。
- 线上健康检查正常，后台未登录接口仍正确返回 401。

### 待完成

- 使用管理员账号完成发布、编辑、单条下架及反馈处理的真实后台操作测试。
- 清理 QA 测试用户及其关联业务记录前需再次确认。
