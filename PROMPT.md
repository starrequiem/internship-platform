...
数据来源：爬虫抓取（牛客汇总站 + 字节/腾讯/美团/阿里/小红书/华为/网易 等大厂 API + 实习僧网页）。
...
### ✅ 已完成（截至 2026-08-17）
- 文本爬取分割优化：`extract.py`（牛客详情/投递时间）、`segment_company.py`（字节整页）、`config.py`（公司名兜底）
- 投递时间 `apply_time` 字段贯通 DB/爬虫/API/前端，无值显示「详见原页面」
- 牛客列表分页翻页、字节 API CSRF token、networkidle→domcontentloaded
- 新增站点：网易（API）、实习僧（SSR + 数字分页翻页）；Boss直聘需登录+滑块，未接入
- 城市扩展 55 个（省会+超一线+新一线+二线）、岗位分类统一 15 类、专业方向 `target_major` 映射
- classify 统一到 `config.py`（`classify_job_type`），各爬虫共用
- 全量重抓 6776 条，公司名归一（字节跳动科技有限公司→字节跳动、牛客页脚公司→待识别）
...
### 已知问题（低优先级）
- 新分类（工科类/硬件芯片/法务合规/新闻媒体/算法AI）数据待下次全量重抓填充
